        ...
        <div className="relative flex flex-col mx-auto text-lg shadow-lg bg-app-black-modal rounded-xl w-158 sm:p-[50px] xs:p-[40px] 1xs:p-[30px] 2xs:p-[20px] p-[10px]">
          ...
          <div className="flex flex-row justify-between py-5 mb-5 rounded-md px-7 bg-app-black">
            ...
            <div className="flex items-center">
              <div className="text-xs font-medium text-blue-700 2xs:text-base">{lpBalance}</div>
            </div>
          </div>
          {!isUnstakeMode && (
            <div className="flex flex-row justify-between py-5 mb-2 rounded-md px-7 bg-app-black">
              ...
              <div className="text-xs font-medium text-blue-700 2xs:text-base">{poolShare}%</div>
            </div>
          )}
          <div className="flex flex-row justify-between py-5 mb-2 rounded-md px-7 bg-app-black">
            <div className="text-xs font-normal 2xs:text-base">
              {isUnstakeMode ? "Received" : "Pooled"} {tokenFrom.title}
            </div>
            <div className="text-xs font-medium text-blue-700 2xs:text-base">{values.from}</div>
          </div>
          <div className="flex flex-row justify-between py-5 mb-5 rounded-md px-7 bg-app-black">
            <div className="text-xs font-normal 2xs:text-base">
              {isUnstakeMode ? "Received" : "Pooled"} {tokenTo.title}
            </div>
            <div className="text-xs font-medium text-blue-700 2xs:text-base">{values.to}</div>
          </div>
          <div
